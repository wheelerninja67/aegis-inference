...
fn print_aegis_banner() {
    ...
    println!(
        "{COLOR_BLUE}   🛡️  AEGIS v6.0.0-rc1 | BARE-METAL INFERENCE ENGINE                {COLOR_RESET}"
    );
    println!(
        "{COLOR_BLUE}========================================================================{COLOR_RESET}"
    );
    println!(
        "{COLOR_YELLOW}[SYS] Target Architecture:   {COLOR_GREEN}AVX2 / NEON (Branchless Bitmask){COLOR_RESET}"
    );
    println!(
        "{COLOR_YELLOW}[SYS] Quantization Protocol: {COLOR_GREEN}1.58-bit Ternary (BitNet b1.58){COLOR_RESET}"
    );
    println!(
        "{COLOR_YELLOW}[SYS] Enterprise Server:     {COLOR_GREEN}Tokio Async Runtime{COLOR_RESET}"
    );
    println!();
}
...
fn simulate_boot_sequence() {
    ...
    println!(
        "{COLOR_GREEN}[*] Mapping physical memory via MAP_POPULATE (Zero-Copy)...{COLOR_RESET}"
    );
    ...
    println!(
        "{COLOR_GREEN}[+] Engine Online. Latency optimized for sub-10ms execution.{COLOR_RESET}"
    );
    println!();
}
...
fn simulate_inference() {
    ...
    println!(
        "{COLOR_YELLOW}[METRICS] Raw Latency:  {COLOR_RED}{:?}{COLOR_RESET}",
        duration
    );
