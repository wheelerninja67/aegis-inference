...
impl AegisEngine {
    pub fn new(
        ...
        eprintln!(
            "[engine] model loaded: {} layers, {} heads, embd={}",
        ...
        eprintln!(
            "[engine] PagePool: {} pages × {} bytes/page",
            total_kv_pages,
        ...
        let _ = rayon::ThreadPoolBuilder::new()
            ...
            .build_global();
        eprintln!(
            "[engine] Rayon pool: {} threads (physical cores)",
            n_physical_cores
        );
    ...
    pub fn step_forward(&mut self) -> Vec<(u32, u32)> {
        ...
        let filtered_outputs: Vec<(u32, u32)> = outputs
            .into_iter()
            .filter(|(seq_id, _)| {
                if let Some(seq) = self
                    .scheduler
    ...
    fn step_batch(&self, sequences: &[SequenceState]) -> Vec<(u32, u32)> {
        ...
        for layer_idx in 0..n_layers {
            ...
            let w_attn_norm =
                self.get_norm_tensor(&format!("{}attn_norm.weight", layer_name_prefix));
                        ...
                        .for_each(|(v, logit)| {
                            ...
                            while i + 8 <= n_embd {
                                ...
                                let b = f32x8::from_slice(
                                    &self.model.embed_table[offset + i..offset + i + 8],
                                );
                                sum_vec += a * b;
                                i += 8;
                            }
    ...
    fn ternary_proj_i8(
        ...
        (0..out_dim)
            ...
            .map(|row| {
                ...
                let raw_dot = unsafe { (vt.ternary_dot)(x.as_ptr(), pm, nm, w.cols) };
